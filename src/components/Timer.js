...
const Timer = ({ time, addTomato }) => {
    ...
    let seconds = (Math.floor(timeRemaining % 60)).toLocaleString('en-US', {minimumIntegerDigits: 2, useGrouping:false})
    ...
    return (
        <div>
            ...
            <button className="btn btn-block" onClick={pauseTimer}>{isPaused && timeRemaining === (time * 60) ? 'Start' : isPaused ? 'Resume' : 'Pause'}</button>         
            <button className="btn btn-block" onClick={resetTimer}>Reset</button>         
        </div>
    )
}
